...
class Controller
{
	 ...
	 */
	final protected function response(Request $request, Response $response, string $view = '', array $data = [])
	{
	 ...
	 */
	final private function buildResponse(array $data, $json = false)
	{
	 ...
	 */
	final private function json(Request $request, array $data = [])
	{
		$data = $this->buildResponse($data);
		if(count($this->errors) === 0) {
			unset($data['errors']);
